               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    ...
    int PARTITIONS = 10;
    int rowNum = (int)dataSizeY / PARTITIONS;
    int jobs[THREAD_NUM]; // currently processing 
    int check[THREAD_NUM]; // waiting to complete
    int *outBuffers[THREAD_NUM];
    MPI_Request requests[THREAD_NUM];
    ...
    // Broadcast constant info
    int baseData[] = {dataSizeX, kernelSizeX, kernelSizeY};
    MPI_Bcast((void*) baseData, 3, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast((void*) kernel, kernelSize, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    int complete = 0;
    int working = 0;
    while(complete < PARTITIONS) {
        for(z = 1; z < THREAD_NUM; z++) {
            if(jobs[z] == -1 && working < PARTITIONS) { 
                // currently without anything to do, give work to the thread
                MPI_Send((void*) &goOnFlag, 1, MPI_INT, z, 0, MPI_COMM_WORLD);
                
                int init = working * rowNum;
                int end = (rowNum * (working+1)) - 1;
                int mInit = init - kCenterY;
                int mEnd = end + kCenterY;
                
                if(mEnd > dataSizeY || PARTITIONS == 1) mEnd = end;
                if(mInit < 0) mInit = 0;
                
                // GROUP
                int data[] = {init, end, mInit, mEnd};  
                MPI_Send((void*)data, 4, MPI_INT, z, 0, MPI_COMM_WORLD);
                
                int matrixSize = (mEnd-mInit+1) * dataSizeX;
                MPI_Send((void*)(in + (mInit*dataSizeX)), matrixSize, MPI_INT, z, 0, MPI_COMM_WORLD);
                
                jobs[z] = working++;
            }
            else {
                // Check for completion
//                 printf("Checking if job %d is complete\n", z);
                if(jobs[z] == -1) continue;
                
                int init = jobs[z] * rowNum;
                int end = (rowNum * (jobs[z]+1)) - 1;
                
                int flag = 0;
                MPI_Status status;
                int dataSize = (end-init+1) * dataSizeX;
                
                if(check[z] == -1) {
                    outBuffers[z] = malloc(sizeof(int)*dataSize);
                    MPI_Irecv((void*)outBuffers[z], dataSize, MPI_INT, z, MPI_ANY_TAG, MPI_COMM_WORLD, &requests[z]);
                    
                    check[z] = 1;
                }
                
                MPI_Test(&requests[z], &flag, &status);
                
                if(flag) {
                    int *buf = outBuffers[z];                    
                    
                    outPtr += init*dataSizeX;
                    int i;
    ...
    for(z = 1; z < THREAD_NUM; z++) {
        MPI_Send((void*) &endFlag, 1, MPI_INT, z, 0, MPI_COMM_WORLD);
    }
}
...
void parallelConvolution() {  
    ...
    int kernelSizeX, kernelSizeY, dataSizeX, kCenterX, kCenterY;
    int baseData[3];
    MPI_Bcast((void*) baseData, 3, MPI_INT, 0, MPI_COMM_WORLD);
    
    dataSizeX = baseData[0];
    kernelSizeX = baseData[1];
    kernelSizeY = baseData[2];
    
//     printf("Thread %d received base data: \n\tdataSizeX: %d\n\tkernelSizeX: %d\n\tkernelSizeY: %d\n", rank, dataSizeX, kernelSizeX, kernelSizeY);
    ...
    float *kernel, *kPtr;
    kernel = malloc(sizeof(float)*kernelSize);
    MPI_Bcast((void*) kernel, kernelSize, MPI_INT, 0, MPI_COMM_WORLD);
    kPtr = kernel;
    
//     printf("Thread %d received kernel\n", rank);
    
    // Read start working
    int endFlag;
    MPI_Recv((void*) &endFlag, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    
    while(endFlag != 1) {
       
        int i, j, m, n;
        int *inPtr, *inPtr2, *outPtr, *out;
        ...
        int data[4];
        MPI_Recv((void*)data, 4, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        init = data[0];
        end = data[1];
        mInit = data[2];
        mEnd = data[3];
        
        int offset = init - mInit;
        init -= mInit;
        end -= mInit;
        mEnd -= mInit;
        mInit = 0;
        ...
        inPtr = malloc(sizeof(int)*matrixSize);
        MPI_Recv((void*)inPtr, matrixSize, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        inPtr += (dataSizeX * offset); // Put pointer at the start of the current data
        inPtr2 = inPtr;
    
        outPtr = malloc(sizeof(int)*matrixSize);
        out = outPtr;
    
        // start convolution
        for(i= init; i < end; ++i)                   // number of rows
        {
            // compute the range of convolution, the current row of kernel should be between these
            rowMax = i + kCenterY;
            rowMin = i - end + kCenterY;
    ...
    //            else *outPtr = 0;
     ...
     // SEND
        MPI_Send((void*) out, dataSize, MPI_INT, 0, 0, MPI_COMM_WORLD);
        free(out);
        MPI_Recv((void*) &endFlag, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
...
//    int headstored=0, imagestored=0, stored;
    ...
    if(rank == 0) {
        ...
        long position=0;
        double start, tstart=0, tend=0, tread=0, tcopy=0, tconv=0, tstore=0, treadk=0, elapsed = 0;
        struct timeval tim;
