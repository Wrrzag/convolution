               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    ...
    inPtr = &in[dataSizeX * kCenterY + kCenterX];  // note that  it is shifted (kCenterX, kCenterY),
    outPtr = out;
    
    // SEND DATA
    int THREAD_NUM;
    MPI_Comm_size(MPI_COMM_WORLD,&THREAD_NUM);
    THREAD_NUM--; // ignore master
    int rowNum = (int)dataSizeY / THREAD_NUM;
    
    int z;
    for(z = 0; z < THREAD_NUM; z++) {
        int init = z * rowNum;
        int end = (rowNum * (z+1)) - 1;
        int mInit = init - kCenterY;
        int mEnd = end + kCenterY;
        
        if(mEnd > dataSizeY || z == THREAD_NUM - 1) mEnd = end;
        if(mInit < 0) mInit = 0;
     
        // GROUP
        int data[] = {init, end, dataSizeX, kernelSizeX, kernelSizeY, mInit, mEnd};
        MPI_Send((void*)data, 7, MPI_INT, z + 1, 0, MPI_COMM_WORLD);
        
        int kernelSize = kernelSizeX*kernelSizeY;
        int matrixSize = (mEnd-mInit+1) * dataSizeX;
        MPI_Send((void*)kernel, kernelSize, MPI_FLOAT, z + 1, 0, MPI_COMM_WORLD);
        MPI_Send((void*)(in + (mInit*dataSizeX)), matrixSize, MPI_INT, z + 1, 0, MPI_COMM_WORLD);
    }
    ...
    for(z = 0; z < THREAD_NUM; z++) {
        ...
        int matrixSize = (end-init+1) * dataSizeX;
        int *result = malloc(sizeof(int)*matrixSize);
        MPI_Recv((void*)result, matrixSize, MPI_INT, z+1, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
void parallelConvolution() {
    ...
    // RECEIVE DATA
    int data[7];
    MPI_Recv((void*)data, 7, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    MPI_Recv((void*)kernel, kernelSize, MPI_FLOAT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    kPtr = kernel;
    ...
    outPtr = malloc(sizeof(int)*dataSize);
    out = outPtr;
// MPI_Send((void*)inPtr, dataSize, MPI_INT, 0, 0, MPI_COMM_WORLD);return;
    // start convolution
    int i, j, m, n;
