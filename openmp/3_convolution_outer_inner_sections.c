               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    ...
    inPtr = inPtr2 = base = &in[dataSizeX * kCenterY + kCenterX];  // note that  it is shifted (kCenterX, kCenterY),
    outPtr = out;
    kPtr = kernel;
    
    // start convolution
    omp_set_nested(1);
#pragma omp parallel for private(i, j, m, n, rowMin, rowMax, colMin, colMax, sum) default(none) shared(inPtr, inPtr2, kPtr, outPtr, out, base, \
    dataSizeY, kCenterY, kCenterX, dataSizeX, kernelSizeX, kernelSizeY, kernel) num_threads(OUTER_THREADS) 
...
#pragma omp parallel for private(j, m, n, colMin, colMax, sum) firstprivate(inPtr, inPtr2, kPtr, outPtr) num_threads(INNER_THREADS) shared(rowMax, rowMin, i, out, base, dataSizeY, kCenterY, kCenterX, dataSizeX, kernelSizeX, kernelSizeY, kernel, baseOut, baseIn)
        for(j = 0; j < dataSizeX; ++j)              // number of columns
        {
            ...
            for(m = 0; m < kernelSizeY; ++m)        // kernel rows
            {
                ...
                inPtr -= dataSizeX;                 // move input data 1 row up
            }
...
#pragma omp parallel sections num_threads(RGB_THREADS)
{
#pragma omp section
   {
