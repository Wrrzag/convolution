               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    ...
    // start convolution
#pragma omp parallel for private(i, j, m, n, rowMin, rowMax, colMin, colMax, sum) default(none) firstprivate(inPtr, inPtr2, kPtr, outPtr, out, base, \
    dataSizeY, kCenterY, kCenterX, dataSizeX, kernelSizeX, kernelSizeY, kernel) 
    for(i= 0; i < dataSizeY; ++i)                   // number of rows
    {
        ...
        {
//             long size = (i * dataSizeX) + j;
//             printf("%d (outPtr = %d v %d)\n", size, *(out + size), *outPtr);
