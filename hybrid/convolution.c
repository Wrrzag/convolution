               ...
               int yFrom, int yTo)
{
    ...
    inPtr2 = &in[dataSizeX * kCenterY + kCenterX];  // note that  it is shifted (kCenterX, kCenterY),
//     outPtr = out;
//     kPtr = kernel;
...
//    printf( "Hi there it's process %d. I'm convoluting from i: %d to %d\n", rank, yFrom, yTo);
    ...
    // start convolution
    #pragma omp parallel for private(i, j, m, n, rowMin, rowMax, colMin, colMax, sum, inPtr, kPtr) default(none) firstprivate(inPtr2, outPtr) shared(out, \
    dataSizeY, kCenterY, kCenterX, dataSizeX, kernelSizeX, kernelSizeY, kernel, yFrom, yTo) num_threads(OMP_THREADS) 
